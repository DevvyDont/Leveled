...
public class HomingArrows implements Listener {
    ...
    public static class HomingArrowTask extends BukkitRunnable {
        ...
        public void run() {
            ...
            if (!arrow.isValid() || arrow.isOnGround() || arrow.getTicksLived() > 20 * 4 * aggression){ this.cancel(); return; }
            // If the target died somehow, give up
            if (target != null && !target.isValid()) { this.cancel(); return; }
            ...
            if (target != null) {
                ...
                if (aggression == 3) {velocityMultiplier *= 1.001;}
                else if (aggression == 2) { velocityMultiplier *= .9; }
                else if (aggression == 1) { velocityMultiplier *= .7; }
                else { velocityMultiplier *= (1 + ((aggression - 3) / 100.)); }
            ...
            // We need to find a target
                ...
                for (Entity entity : arrow.getWorld().getNearbyEntities(arrow.getLocation(), aggression * 9, aggression * 9, aggression * 9)){
                    ...
                    if (entity.equals(arrow.getShooter())) { continue; }
                    if (target == null &&  ((LivingEntity) entity).hasLineOfSight(arrow) ) { target = (LivingEntity) entity; }
                    else if (target == null) { }
    ...
    public void onArrowShoot(EntityShootBowEvent event){
        ...
        if (event.getEntity() instanceof Player && event.getProjectile() instanceof Arrow){
            ...
            int homingLevel;
            try {homingLevel = CustomEnchantments.getEnchantLevel(player.getInventory().getItemInMainHand(), CustomEnchantType.HOMING); } catch (IllegalArgumentException ignored) { return; }

            int tickFrequency = 4 - homingLevel;
            if (tickFrequency <= 0) { tickFrequency = 1; }
            new HomingArrowTask((Arrow) event.getProjectile(), homingLevel).runTaskTimer(plugin, Math.round(event.getForce() * 12), 4 - (homingLevel));
        }
